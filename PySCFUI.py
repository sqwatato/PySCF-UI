...
bse_pyscf = st.radio("Source of Bais Sets",['PySCF','BSE'])
if bse_pyscf == 'PySCF':
...
# verbose_option = st.selectbox("Verbose", index=2, options=[
    ...
    st.write("*The available Basis Stack Exchange libraries are valid for C, H, O, N, F, S, and P. While they also work for many other atoms, some atoms may not have a corresponding set with the selected basis, in which case the UI will return an error.*")
...
with tab1:
    if 'results' in st.session_state:
        ...
        for result_item in st.session_state['results']:
            ...
            with st.expander(str(st.session_state['results'].index(data) + 1) + "." + data['Molecule Name'] + " | "+data['Basis']+": " + str(round(data['Real Compute Time'], 2)) + " s"):
