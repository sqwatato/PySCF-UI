...
# verbose_option = st.selectbox("Verbose", index=2, options=[
    ...
    st.write("*The available Basis Stack Exchange libraries are available for C, H, O, N, F, S, and P. Future updates will extend this to include all atoms up to chlorine.*")
...
with tab1:
    if 'results' in st.session_state:
        ...
        for result_item in st.session_state['results']:
            ...
            with st.expander(str(st.session_state['results'].index(data) + 1) + "." + data['Molecule Name'] + " | "+data['Basis']+" (" + data['Basis Source']+"): " + str(round(data['Real Compute Time'], 2)) + " s"):
