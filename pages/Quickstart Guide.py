...
st.write("PySCF UI also allows for users to select basis sets from the Basis Set Exchange. The Basis Set Exchange is the most prominent and peer-reviewed basis set database in the field of quantum chemistry and provides access to hundreds of sets. The UI includes basis sets that are available for C, H, N, O, F, S, and P. Future updates will extend this to all atoms up to Cl as well.")
